...
# ========= NEW HELPER: Fetch Circle/Carrier Data =========
def get_carrier_info(num):
    try:
        url = f"https://located-surgeon-breed-schemes.trycloudflare.com//?num={num}"
...
def parse_db_message(text):
    ...
    return {"database": lines[0], "details": [l for l in lines if ":" in l]}
    ...
    try:
        ...
        while True:
            try:
                ...
                if parsed:
                    if parsed['database'] in seen_titles: break
...
async def api_call(query: str = Query(None)):
    ...
    # Step 1: Get Carrier/Circle Data first
    carrier_data = get_carrier_info(query)
    
    # Step 2: Get Telegram Scrape Data
    telegram_results = await scrape_all(query)
